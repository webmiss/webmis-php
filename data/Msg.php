...
class Msg extends Base {
  ...
  /* 列表 */
  static function GetList(int $uid, string $title='', int $days=30): array {
    // 近30天
    ...
    $num = 0;
    $m = new UserMsg();
    $m->Columns('id', 'gid', 'uid', 'fid', 'format', 'title', 'content', 'is_new', 'FROM_UNIXTIME(ctime) as time');
    ...
    foreach($all as $v){
      // 未读消息
      $n = !in_array($uid, json_decode($v['is_new']))?1:0;
      $num += $n;
      // 系统信息
      if($v['gid']){

      }else{
        $tid = $v['uid']!=$uid?$v['uid']:$v['fid'];
        if(!isset($list[$tid])) $list[$tid] = ['num'=>0, 'list'=>[]];
        $list[$tid]['type'] = 'msg';
        $list[$tid]['gid'] = 0;
        $list[$tid]['fid'] = $tid;
        $list[$tid]['num'] += $n;
        $list[$tid]['title'] = $title?$uinfo[$tid][$title]:$v['title'];
        $list[$tid]['time'] = $v['time'];
        $list[$tid]['img'] = sData::Img($uinfo[$tid]['img']);
        ...
        $list[$tid]['list'][] = [
          ...
          'fid'=> $v['fid'],
          'is_new'=> $n?true:false,
          'title'=> $title?$uinfo[$tid][$title]:$v['title'],
          'img'=> sData::Img($uinfo[$v['fid']]['img']),
          'time'=> $v['time'],
          'content'=> $v['content'],
        ];
      }
