...
class ErpBaseSupplier extends Base {
  ...
  // 状态
  static private $state_name = ['1'=>'正常', '0'=>'禁用'];
  // 导出
  ...
  static function List(): string {
    ...
    // 统计
    $m = new ErpBaseSupplierM();
    $m->Columns('id', 'supplier_id', 'name', 'state', 'tel', 'city', 'depositbank', 'bankacount', 'acountnumber', 'alipay_id', 'alipay_name', 'remark', 'operator_name', 'FROM_UNIXTIME(ctime) as ctime', 'FROM_UNIXTIME(utime) as utime', 'FROM_UNIXTIME(btime) as btime');
    $m->Where($where);
  ...
  static private function getWhere(array $d): string {
    ...
    // 状态
    $state = isset($d['state'])&&!empty($d['state'])?$d['state'][0]:'';
  ...
  static function Export(): string {
    ...
    // 查询
    $m = new ErpBaseSupplierM();
    $m->Columns('id', 'supplier_id', 'name', 'state', 'tel', 'city', 'depositbank', 'bankacount', 'acountnumber', 'alipay_name', 'alipay_id', 'operator_name', 'remark', 'FROM_UNIXTIME(ctime) as ctime', 'FROM_UNIXTIME(utime) as utime');
    $m->Where($where);
  ...
  static function GetSelect(): string {
    ...
    // 返回
    return self::GetJSON(['code'=>0, 'data'=>['city_name'=> $city_name, 'state_name'=>$state_name]]);
  }
  ...
  static function GetInfo(): string {
    ...
    $data = [];
    $m = new ErpBaseSupplierM();
    $m->Columns('id', 'supplier_id', 'name', 'state', 'tel', 'depositbank', 'bankacount', 'acountnumber', 'alipay_id', 'alipay_name', 'remark', 'FROM_UNIXTIME(ctime) as ctime', 'FROM_UNIXTIME(utime) as utime', 'FROM_UNIXTIME(btime) as btime');
    $m->Where('state="1" AND name=?', trim($name));
    $data = $m->FindFirst();
