...
class ErpGoodsInfo extends Base {
  ...
  static function Import(): string {
    ...
    // 上传聚水谭
    $list = array_chunk($data, 200, false);
    foreach ($list as $v) {
      Stock::JuShuiTanGoods(json_encode($v));
      // Task::Popen('php ../cli.php Stock JuShuiTanGoods', $v);
      sleep(1);
    }
    // 返回
  ...
  static function GetSelect(): string {
    ...
    // 标签
    $labels = [];
    foreach (Goods::$labels as $v) $labels[] = ['label'=> $v, 'value'=> $v];
    // 分类
    $category_name = [];
