...
use Data\Goods;
use Util\Util;
use Model\ErpPurchaseOut as ErpPurchaseOutM;
use Model\ErpPurchaseOutShow;

class ErpPurchaseOut extends Base {
  ...
  static function Explode(): string {
    ...
    if(!$info) return self::GetJSON(['code'=>4000, 'msg'=>'当前状态不可用!']);
    // 明细
    $pname = sData::PartitionName($info['ctime'], $info['utime']);
    $m = new ErpPurchaseOutShow();
    if($pname) $m->Partition($pname);
    $m->Columns('id', 'sku_id', 'num');
    $m->Where('pid=?', $id);
    $all = $m->Find();
    if(!$all) return self::GetJSON(['code'=>4000, 'msg'=>'商品不能为空!']);
    $sku = [];
    foreach($all as $v) $sku[$v['sku_id']] = ['id'=>$v['id'], 'num'=>$v['num']];
    // 商品资料
    $goods = Goods::GoodsInfoAll(array_keys($sku), 'data', [-1, 3], ['sku_id', 'brand', 'cost_price', 'sale_price', 'purchase_price', 'market_price', 'ratio', 'ratio_cost', 'ratio_sale', 'ratio_purchase', 'ratio_market']);
    // 品牌分组
    $brand = [];
    foreach($goods as $k=>$v) {
      if(!isset($brand[$v['brand']])) {
        $brand[$v['brand']]['id'] = [];
        $brand[$v['brand']]['cost_price'] = 0;
        $brand[$v['brand']]['sale_price'] = 0;
        $brand[$v['brand']]['purchase_price'] = 0;
        $brand[$v['brand']]['market_price'] = 0;
        $brand[$v['brand']]['num'] = 0;
        $brand[$v['brand']]['total'] = 0;
      }
      $brand[$v['brand']]['id'][] = $sku[$k]['id'];
      $brand[$v['brand']]['cost_price'] += $v['cost_price']*$sku[$k]['num']*($v['ratio']<1?$v['ratio']:$v['ratio_cost']);
      $brand[$v['brand']]['sale_price'] += $v['sale_price']*$sku[$k]['num']*($v['ratio']<1?$v['ratio']:$v['ratio_sale']);
      $brand[$v['brand']]['purchase_price'] += $v['purchase_price']*$sku[$k]['num']*($v['ratio']<1?$v['ratio']:$v['ratio_purchase']);
      $brand[$v['brand']]['market_price'] += $v['market_price']*$sku[$k]['num']*($v['ratio']<1?$v['ratio']:$v['ratio_market']);
      $brand[$v['brand']]['num'] += $sku[$k]['num'];
      $brand[$v['brand']]['total'] += 1;
    }
    // 更新
    $i = $pid = 0;
    $admin = AdminToken::Token($token);
    foreach($brand as $k=>$v) {
      ...
      if($i===0) $pid = $info['id'];
        ...
        $m->Values([
          ...
          'remark'=> $info['remark'],
          'ctime'=>time(),
          'creator_id'=> $admin->uid,
          'creator_name'=> $admin->name,
        ]);
        if($m->Insert()) $pid = $m->GetID();
        else return self::GetJSON(['code'=>4000, 'msg'=>'创建失败!']);
      }
      $i++;
      // 明细
      $m1 = new ErpPurchaseOutShow();
      if($pname) $m1->Partition($pname);
      ...
      $m2->Where('id=?', $pid);
      // 失败
      if(!$m1->Update() || !$m2->Update()) return self::GetJSON(['code'=>4000, 'msg'=>'更新失败!']);
    }
    return self::GetJSON(['code'=>0]);
  }
  ...
  static function Export(): string {
    ...
    // 文件名
    $admin = AdminToken::Token($token);
    $file_name = 'PurchaseOut_'.date('YmdHis').'_'.$admin->uid.'.xlsx';
