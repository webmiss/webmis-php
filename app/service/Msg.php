...
class Msg {
  ...
  static function GetList(int $uid, int $days=30): array {
    ...
    foreach($all as $v) {
      ...
      if(!isset($list[$fid])) {
        ...
        // 群组
        if($v['gid']) {
          $list[$fid]['title'] = $cfg['info'][$v['gid']]['title'];
          $list[$fid]['img'] = $cfg['info'][$v['gid']]['img'];
        }
      } else {
        $list[$fid]['num'] += $n;
      }
  ...
  static function GetShow(int $gid, int $fid, int $uid, int $page=1, int $limit=30): array {
    ...
    foreach($list as $k=>$v) {
      ...
      // 群组
      if($gid && $v['fid']===0) {
        $list[$k]['title'] = $cfg['info'][$gid]['title'];
        $list[$k]['img'] = $cfg['info'][$gid]['img'];
      }
    }
  ...
  /* 搜索联系人 */
  static function SeaUser(int $uid, string $key, string $where='a.type<>1'): array {
    // 配置
    $cfg = Socket::config();
    // 查询
    $m = new UserInfo();
    ...
    $m->Where($where.' AND a.'.self::$name.' like ? AND a.uid<>? AND b.state="1"', '%'.$key.'%', $uid);
    $m->Limit(0,10);
    $list = $m->Find();
    foreach($list as $k=>$v){
      $list[$k]['gid'] = 0;
    ...
    if(!$list) $list = array_merge([
      ['gid'=>1, 'fid'=>0, 'uid'=>1, 'title'=>$cfg['info'][1]['title'], 'img'=>$cfg['info'][1]['img']],
    ], $list);
    return $list;
  }
