...
class Goods extends Controller {
  ...
  static function GoodsInfo(string $sku_id, string $type='data', array $columns=[]): array|string {
    $pname = Data::PartitionSku($sku_id, [0, 1]);
  ...
  static function GoodsInfoAll(array $sku, string $type='data', array $limit=[0, 1], array $columns=[]): array|string {
    ...
    foreach ($sku as $sku_id) {
      $pname = Data::PartitionSku($sku_id, $limit);
      if(!$pname) {
        $tmp=[]; break;
      }
  ...
  static function GoodsInfoVerify(array $data): string|array {
    ...
    // 品牌
    if(empty(self::$Brand)) self::$Brand = ErpBaseBrand::GetList();
    // 分类
    self::$category = [];
  ...
  */
  ...
  static function GoodsWork(string $sku_id, array $limt = []): array|string {
    // 分区
    $info = self::GoodsInfo($sku_id);
    if(!$info) return [[], []];
    $info['num'] = '';
    $info['img'] = $info['img']?Data::ImgGoods($info['sku_id'], false):'';
    $pname = Data::PartitionSku($sku_id, $limt);
    if(!$info) return $info;
    // 统计
    $total = [];
    // 库存
    ...
    // 数据
    $partner_all = ErpBasePartner::GetList();
    foreach ($list as $k => $v) {
      // 商品信息
