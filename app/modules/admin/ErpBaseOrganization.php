...
use App\Util\Util;
use App\Model\ErpBaseOrganization as ErpBaseOrganizationM;

/* 组织架构 */
class ErpBaseOrganization extends Controller {
  ...
  static function Export() {
    ...
    // 导出文件
    $admin = TokenAdmin::Token($token);
    self::$export_filename = 'ErpOrganization_'.date('YmdHis').'_'.$admin->uid.'.xlsx';
    $html = Export::ExcelTop();
    $html .= Export::ExcelTitle([
      'ID', 'FID', '名称', '排序', '状态', '创建时间', '更新时间', '制单员', '操作员', '备注'
    ]);
    // 数据
    foreach($list as $k=>$v){
      // 内容
